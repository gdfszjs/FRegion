...
{	
	Constraint c(nHandle, Wml::Vector2f(0.0f,0.0f));
...
void RigidMeshDeformer2D::UpdateConstraint( Constraint & cons )
{
	...
	if ( found != m_vConstraints.end() ) {
		Constraint ccc(*found);
		ccc.vConstrainedPos = cons.vConstrainedPos;
		m_vConstraints.erase(found);
		m_vConstraints.insert(ccc);
...
void RigidMeshDeformer2D::PrecomputeFittingMatrices()
{
	...
	for ( unsigned int i = 0; i < nVerts; ++i ) {
		Constraint c(i, Wml::Vector2f(0.0f,0.0f));
